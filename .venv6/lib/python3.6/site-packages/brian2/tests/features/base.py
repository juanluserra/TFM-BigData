...
class SpeedTestResults(object):
    ...
    def get_ns(self, fullname):
        L = [(cn, fn, n, s) for cn, fn, n, s in list(self.full_results.keys()) if fn==fullname]
        confignames, fullnames, n, codeobjsuffixes  = list(zip(*L))
...
def make_table(grid):
    max_cols = [max(out) for out in map(list, list(zip(*[[len(item) for item in row] for row in grid])))]
