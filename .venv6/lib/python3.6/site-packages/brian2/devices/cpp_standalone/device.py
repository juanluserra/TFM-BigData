...
class CPPStandaloneDevice(Device):
        ...
        if isinstance(var, DynamicArrayVariable):
            ...
            if var.ndim == 1:
                ...
                raise AssertionError(('Did not expect a dynamic array with %d '
                                      'dimensions.') % var.ndim)
            while (dynamic_name in list(dynamic_dict.values()) or
                   array_name in list(self.arrays.values())):
                suffix += 1
            ...
            orig_array_name = array_name = '_array_%s_%s' % (var.owner.name, var.name)
            suffix = 0
            while (array_name in list(self.arrays.values())):
                suffix += 1
                array_name = orig_array_name + '_%d' % suffix
            self.arrays[var] = array_name
    ...
    def write_static_arrays(self, directory):
        ...
        for code_object in self.code_objects.values():
            for name, value in code_object.variables.items():
    ...
    def run(self, directory, with_output, run_args):
        with in_directory(directory):
            ...
            for key, value in prefs.devices.cpp_standalone.run_environment_variables.items():
            ...
            if os.path.isfile('results/last_run_info.txt'):
                ...
                self._last_run_time, self._last_run_completed_fraction = list(map(float, last_run_info.split()))
