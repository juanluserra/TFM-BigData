...
def make_statements(code, variables, dtype, optimise=True, blockname=''):
    ...
    subexpressions = dict((name, val) for name, val in list(variables.items()) if isinstance(val, Subexpression))
    ...
    subexpr_deps = dict((name, [dep for dep in subexpr.identifiers if dep in subexpressions]) for \
                                                            name, subexpr in list(subexpressions.items()))
    sorted_subexpr_vars = topsort(subexpr_deps)

    statements = []

    # none are yet defined (or declared)
    subdefined = dict((name, False) for name in list(subexpressions.keys()))
    for line in lines:
        stmt = line.statement
        read = line.read
        write = line.write
