...
def dimensions_and_type_from_string(unit_string):
    ...
    if _base_units_with_alternatives is None:
        base_units_for_dims = {}
        for unit_name, unit in reversed(list(DEFAULT_UNITS.items())):
            if float(unit) == 1.0 and repr(unit)[-1] not in ['2', '3']:
                if unit.dim in base_units_for_dims:
                    ...
                    if unit_name != repr(unit):
                        base_units_for_dims[unit.dim].append(unit_name)
        alternatives = sorted([tuple(values) for values in base_units_for_dims.values()])
...
class Equations(collections.Mapping):
    ...
    def __init__(self, eqns, **kwds):
        ...
        for varname, replacement in kwds.items():
    ...
    ordered = property(lambda self: sorted(iter(self._equations.values()),
